...
struct FmhaFwdKernel
{
    ...
    CK_TILE_HOST static std::string GetName()
    {
        ...
        return
            ...
            (kDoFp8StaticQuant ? "_squant" : "_nsquant" ) + (kFp8DQuant ? "_dquant" : "_ndquant" );
        #undef _SS_
        #undef _TS_
        // clang-format on
    }
    ...
    CK_TILE_DEVICE void operator()(Kargs kargs) const
    {
        ...
        const index_t offset_descale_q = i_batch * kargs.batch_stride_descale_q + 
            i_nhead * kargs.nhead_stride_descale_q + 
            ck_tile::integer_divide_ceil(i_m0, 128) * kargs.stride_descale_q;

        const index_t offset_descale_k = i_batch * kargs.batch_stride_descale_k + 
            i_nhead * kargs.nhead_stride_descale_k;
        const index_t offset_descale_v = i_batch * kargs.batch_stride_descale_v + 
            i_nhead * kargs.nhead_stride_descale_v;
        // if (threadIdx.x==0)// && blockIdx.y==0)
        // printf("FmhaFwdKernel blockIdx.y=%u, i_tile_m=%d, i_m0=%d,"
        //     " offset_descale_q=%d.\n",
        //     blockIdx.y, i_tile_m, i_m0, offset_descale_q);
        if constexpr(0)
        // if (threadIdx.x==0)// && blockIdx.y==0)
        // printf("FmhaFwdKernel descale q(%d, %d, %d): %f, %f, %f.\n", i_batch, i_nhead, i_tile_m,
        printf("FmhaFwdKernel descale q(%d, %d, %d): %f.\n", i_batch, i_nhead, i_tile_m,
            *(kargs.descale_q_ptr + offset_descale_q)
            // *(kargs.descale_k_ptr + offset_descale_k),
            // *(kargs.descale_v_ptr + offset_descale_v)
        );
        ...
        auto o_acc_tile = [&]() {
            if constexpr(kDoFp8StaticQuant || kFp8DQuant)
            {
                return FmhaPipeline{}(
                    ...
                    composes(saturates<ODataType>{}, scales{kargs.scale_o}), // o_acc_element_func
                    *(kargs.descale_q_ptr + offset_descale_q),
                    kargs.descale_k_ptr + offset_descale_k,
                    kargs.descale_v_ptr + offset_descale_v,
