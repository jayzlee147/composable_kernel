                                         ...
                                         const ACCElementOp1& acc_element_op1 = {})
{
    ...
    auto f = [&](auto batch, auto m) {
        for(std::size_t n = 0; n < N; ++n)
        {
            ...
            for(std::size_t k1 = 0; k1 < K; k1+=128)
            {
                AccDataType v_acc0 = 0;
                for(std::size_t k0 = 0; k0 < 128; ++k0)
                {
                    std::size_t k = k1 + k0;
                    ADataType v_a = a_element_op(std::make_tuple(batch, m, k), a_b_m_k(batch, m, k));
                    BDataType v_b = b_element_op(std::make_tuple(batch, n, k), b_b_n_k(batch, n, k));

                    // ADataType v_a = a_element_op(a_b_m_k(batch, m, k));
                    // BDataType v_b = b_element_op(b_b_n_k(batch, n, k));

                    v_acc0 += ck_tile::type_convert<AccDataType>(v_a) *
                            ck_tile::type_convert<AccDataType>(v_b);
                }
                v_acc += acc_element_op0(std::make_tuple(batch, m, n, k1), v_acc0);
            }
