...
bool run(const ck_tile::ArgParser& arg_parser)
{
    ...
    // ck_tile::Show<float>{}(descale_q_host);
    ...
    for(ck_tile::index_t wb = 0; wb < batch; ++wb)
    {
        ...
        ck_tile::reference_batched_gemm<QDataType, KDataType, SaccDataType, SMPLComputeDataType>(
            ...
            //     return value * descale_q_host(wb, std::get<0>(idx), std::get<1>(idx)/128);
            // },
            // [&descale_k_host, wb](auto idx, auto value) {
            //     return value * descale_k_host(wb, std::get<0>(idx), std::get<1>(idx)/128);
            // },
