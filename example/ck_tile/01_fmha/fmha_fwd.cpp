...
bool run(const ck_tile::ArgParser& arg_parser)
{
    ...
    for(ck_tile::index_t wb = 0; wb < batch; ++wb)
    {
        ...
        ck_tile::reference_batched_gemm<QDataType, KDataType, SaccDataType, SMPLComputeDataType>(
            ...
            [](auto, auto v, auto, auto, auto) {return v;},
            [&descale_q_host, &descale_k_host, scale_s, wb](auto idx, auto value) {
                auto res = value * scale_s * 
                    // idx: n, sq, sk
                    descale_q_host(wb, std::get<0>(idx), std::get<1>(idx)/128) *   
                    descale_k_host(wb, std::get<0>(idx), std::get<2>(idx)/128);
                // printf("s_host_ref(%lu, %lu, %lu), res(%f), value(%f), scale_s(%f), descale_q_host(%f), descale_k_host(%f).\n", 
                //     std::get<0>(idx), std::get<1>(idx), std::get<2>(idx),
                //     res, value, scale_s,  descale_q_host(wb, std::get<0>(idx), std::get<1>(idx)/128),
                //     descale_k_host(wb, std::get<0>(idx), std::get<2>(idx)/128)
                // );
                return res;
            }
        ...
        ck_tile::reference_batched_gemm<PDataType, VDataType, OaccDataType, ODataType>(
            ...
            [&descale_v_host, wb](auto idx, auto value, auto acc_value, auto a, auto b) {
                // printf("HO(%lu, %lu, %lu, %lu), %f, %f, %f, %f.\n", 
                //     std::get<0>(idx), std::get<1>(idx), std::get<2>(idx), std::get<3>(idx), 
                //     value, acc_value, a, b);
